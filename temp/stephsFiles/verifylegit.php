<?php
	...
	/*$address_regex = "/([0-9]{4}\s[a-z0-9]+)/";
	if (preg_match($address_regex, $_POST["address"]))
		echo "Addresses must begin with 4 digits followed by a whitespace. ";*/
	if (strlen($_POST["address"]) < 6)
		echo "Addresses must be at least 6 characters. ";
	...
	/*try {
		$_POST["email"] = new MailAddress($_POST["email"]).Address;
	} catch (FormatException $_POST) {
